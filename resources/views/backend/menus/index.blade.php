...
@section('content')
    ...
    <p class="text-muted">
        ...
        header item that only opens a dropdown. Lower order numbers show first. The footer title and text are in
                            ...
                            <table class="table table-sm mb-0 menu-table">
                                ...
                                <tbody>
                                    @forelse ($items as $item)
                                        @include('backend.menus.row', ['item' => $item, 'location' => $location, 'parents' => $items, 'child' => false])
                                        @foreach ($item->children as $childItem)
                                            @include('backend.menus.row', ['item' => $childItem, 'location' => $location, 'parents' => $items, 'child' => true])
                                        @endforeach
                                    @empty
...
@section('css')
    <style>
        ...
        .menu-table tr.is-hidden input[type=text] { color: #999; }
    </style>
@stop
